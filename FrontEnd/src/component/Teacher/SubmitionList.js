...
// Main component rendering all submissions
const SubmissionListMain = ({ submissions }) => {
    return (
        <Box>
            <Typography
                variant="h5"
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center' ,gap:'1rem' }}>
                {submissions && submissions.length > 0 ? (
                    submissions.map((submission, index) => (
                        <SubmissionCard key={submission.subId} submission={submission} index={index + 1} />
                    ))
                ) : (
                    <Grid item>
                        <Card
...
export default SubmissionListMain;
