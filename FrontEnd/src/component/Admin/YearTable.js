...
const Table = () => {
  ...
  const URL = `${process.env.REACT_APP_API_URL}/Years`
    ...
    () => [
      ...
      },
      {
        ...
        muiEditTextFieldProps: {
          required: true,
          error: !!validationErrors?.yearName,
      ...
      },
      {
        ...
        muiEditTextFieldProps: (row) => ({
          type: 'date',
          value: convertDateToInputFormat(row,row.row.original.dateStart),
          onChange: (event) => {
            const newDate = event.target.value;
            row.row.original.dateStart = newDate;
          },
          required: true,
      ...
      },
      {
        ...
        muiEditTextFieldProps: (row) => ({
          type: 'date',
          value: convertDateToInputFormat(row,row.row.original.dateEnd),
          onChange: (event) => {
            const newDate = event.target.value;
            row.row.original.dateEnd = newDate;
          },
          required: true,
  ...
  const openDeleteConfirmModal = async (row) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      ...
      try {
        ...
        setFetchedUsers(fetchedUsers.filter((d) => d.yearId !== row.original.yearId));
...
const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Add Year</DialogTitle>
        <DialogContent
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Edit Year</DialogTitle>
        <DialogContent
...
function useGetUsers(setFetchedUsers) {
  ...
  return useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Years`); // replace with actual API endpoint
      setFetchedUsers(response.data.data)
...
const validateYear = (year) => {
    ...
    if (!year.yearName) errors.yearName = 'Year name is required';
    if (!year.dateStart) errors.dateStart = 'Start date is required';
    if (!year.dateEnd) errors.dateEnd = 'End date is required';
    
    // Year name validation
    if (year.yearName && !/^\d{4}$/.test(year.yearName)) {
      errors.yearName = 'Year name must be a valid 4-digit year';
    }
  
    // Date validation
    const today = new Date().toISOString().split('T')[0]; // Get today's date in YYYY-MM-DD format
    const dateStart = new Date(year.dateStart);
    const dateEnd = new Date(year.dateEnd);
    ...
    if (year.dateStart && year.dateEnd) {
        ...
        if (dateEnd < dateStart) {
          ...
          const dayDifference = (dateEnd - dateStart) / (1000 * 60 * 60 * 24);
          if (dayDifference > 365) {
            errors.dateEnd = 'The gap between start date and end date should be 365 days or less';
          }
        }
    }
