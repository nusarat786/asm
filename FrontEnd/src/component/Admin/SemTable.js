...
const Table = () => {
  ...
  const URL = `${process.env.REACT_APP_API_URL}/Semesters`
...
//   };
    ...
    () => [
      ...
      },
      {
        ...
        muiEditTextFieldProps: {
          required: true,
          error: !!validationErrors?.semName,
          helperText: validationErrors?.semName,
          onFocus: () =>
            setValidationErrors({
              ...validationErrors,
              semName: undefined,
  ...
  const handleCreateUser = async ({ values, table }) => {
    const newValidationErrors = validateSem(values);
  ...
  const openDeleteConfirmModal = async (row) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      ...
      try {
        ...
        setFetchedUsers(fetchedUsers.filter((d) => d.semId !== row.original.semId));
...
const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Add Sem</DialogTitle>
        <DialogContent
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Edit Sem</DialogTitle>
        <DialogContent
...
function useGetUsers(setFetchedUsers) {
  ...
  return useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Semesters`); // replace with actual API endpoint
      setFetchedUsers(response.data.data)
...
const validateSem = (sem) => {
    ...
    if (!sem.semName) errors.semName = 'Semester name is required';
    //if (!sem.ts) errors.ts = 'Timestamp is required';
  
    // Semester name validation
    if (sem.semName && !/^SEM-\d+$/.test(sem.semName)) {
      errors.semName = 'Semester name must follow the pattern SEM-X, where X is a number';
    }
  
    // Timestamp validation
    const timestamp = new Date(sem.ts);
    const today = new Date();
