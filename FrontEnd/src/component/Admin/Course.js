    ...
    () => [
      ...
      },
      {
        ...
        muiEditTextFieldProps: {
          ...
          helperText: validationErrors?.cName,
          onFocus: () =>
            setValidationErrors({
              ...validationErrors,
              cName: undefined,
  ...
  const openDeleteConfirmModal = async (row) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      ...
      try {
        ...
        setFetchedUsers(fetchedUsers.filter((d) => d.cId !== row.original.cId));
...
const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Add Course</DialogTitle>
        <DialogContent
...
function useGetUsers(setFetchedUsers) {
  ...
  return useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Courses`); // replace with actual API endpoint
      setFetchedUsers(response.data.data)
...
const validateSem = (c) => {
    ...
    if (!c.cName) errors.cName = 'Course name is required';
