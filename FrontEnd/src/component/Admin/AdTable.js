...
const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Add SuperAdmin</DialogTitle>
        <DialogContent
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Edit Admin</DialogTitle>
        <DialogContent
...
function useGetUsers(setFetchedUsers) {
  ...
  return useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/SuperAdmin`); // replace with actual API endpoint
      setFetchedUsers(response.data.data)
...
const validateAdmin = (values) => {
    ...
    if (!values.password) {
      ...
      !/(?=.*\d)/.test(values.password) ||    // At least one digit
      !/(?=.*[!@#$%^&*])/.test(values.password) // At least one special character
    ) {
