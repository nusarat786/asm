...
const Table = () => {
  ...
  const URL = `${process.env.REACT_APP_API_URL}/CourseOffered`
    ...
    () => [
...

      {
        ...
        muiEditTextFieldProps: {
          required: true,
          select: true,
          error: !!validationErrors?.tid,
          helperText: validationErrors?.tid,
          onFocus: () =>
            ...
            children:  teachers.map((data) => (
              <MenuItem 
              key={data.tid} 
              value={data.tid}>
                  {data.tid + " : " + data.tsname + " " + data.tfname }
              </MenuItem>
          )),
        },
      },
     ...
     
      {
        ...
        muiEditTextFieldProps: {
          required: true,
          select: true,
          error: !!validationErrors?.sid,
          helperText: validationErrors?.sid,
          onFocus: () =>
            setValidationErrors((prev) => ({
              ...prev,
              sid: undefined,
  ...
  const openDeleteConfirmModal = async (row) => {
    if (window.confirm('Are you sure you want to delete this record?')) {
      ...
      try {
        ...
        setFetchedUsers(fetchedUsers.filter((d) => d.coId !== row.original.coId));
...
const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Add Course Offred</DialogTitle>
        <DialogContent
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Edit Course Offred</DialogTitle>
        <DialogContent
...
function useGetUsers(setFetchedUsers) {
  ...
  return useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/CourseOffered`); // replace with actual API endpoint
      setFetchedUsers(response.data.data)
...
const validateYear = (c) => {
    ...
    if (!c.tid) errors.tid = 'Teacher id is required';
    if (!c.sid) errors.sid = 'Subject id is required';
...
export default CourseOffredTable;