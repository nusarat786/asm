    ...
    () => [
      ...
      },
      {
        ...
        muiEditTextFieldProps: {
          ...
          helperText: validationErrors?.sName,
          onFocus: () =>
            setValidationErrors({
              ...validationErrors,
              cName: undefined,
  ...
  const openDeleteConfirmModal = async (row) => {
    if (window.confirm('Are you sure you want to delete this user?')) {
      ...
      try {
        ...
        setFetchedUsers(fetchedUsers.filter((d) => d.sId !== row.original.sId));
...
const table = useMaterialReactTable({
    ...
    renderCreateRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Add Subject</DialogTitle>
        <DialogContent
    ...
    renderEditRowDialogContent: ({ table, row, internalEditComponents }) => (
      <>
        <DialogTitle variant="h3">Edit Sbject</DialogTitle>
        <DialogContent
...
function useGetUsers(setFetchedUsers) {
  ...
  return useQuery({
    queryKey: ['users'],
    queryFn: async () => {
      const response = await axios.get(`${process.env.REACT_APP_API_URL}/Subjects`); // replace with actual API endpoint
      setFetchedUsers(response.data.data)
...
const validateSem = (c) => {
    ...
    if (!c.sName) errors.sName = 'Subject name is required';
